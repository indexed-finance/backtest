...
export function bsub(a: BigNumber, b: BigNumber): BigNumber {
  const { res, bool } = bsubSign(a, b);
  if (bool) throw Error('ERR_SUB_UNDERFLOW');
  return res;
}
...
export function bpow(base: BigNumber, exp: BigNumber, maxIterationsApprox?: number): BigNumber {
  ...
  let partialResult = bpowApprox(base, remain, BPOW_PRECISION, maxIterationsApprox);
...
function bpowApprox(
  ...
  for (let i = 1; term.gte(precision); i++) {
    if (maxIterations && i > maxIterations) {
      // console.log(`Got stuck in infinite loop. i=${i}; sum=${sum.toString()}`);
      throw Error();
    }
    let bigK = new BigNumber(i).times(BONE);
    let { res: c, bool: cneg } = bsubSign(a, bsub(bigK, BONE));
